...
- **Recurrentes** (`Recurrentes.jsx` + `lib/useRecurrentes.js`, localStorage con pub/sub; badge de pendientes en la pestaña): movimientos que se repiten cada mes (alquiler, nómina, suscripciones). Se configuran una vez y se registran cada mes sin rellenar nada. Cada recurrente puede llevar el **día del mes** en que ocurre (alquiler día 1, nómina día 28); los **pendientes** se ordenan por día y distinguen lo que **"toca ya"** (día pasado, resaltado) de lo que llega **"el día N"** (atenuado). Cada uno muestra su **estado del mes**: "✓ Registrado este mes" / "Pendiente este mes" / "Pausado". Los **gastos** se registran automáticos de un toque; los de **importe variable** (nómina) piden **confirmar el importe** antes de registrar, para no meter cifras a ciegas. Pausar/activar, editar y borrar. El movimiento registrado va a Supabase (`es_fijo=true`).
